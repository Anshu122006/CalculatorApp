...
class Calculus {
  ...
  static Future calculateIntegral(String expression, String type) async {
    ...
    switch (type) {
      case "derivative":
        url =
            "$baseUrl?input=d/dx($expression)&format=plaintext&output=JSON&appid=$wolframApiKey";
        break;
      case "solve":
        url =
      ...
      default:
        url =
            "$baseUrl?input=integrate+$expression&format=plaintext&output=JSON&appid=$wolframApiKey";
    }
